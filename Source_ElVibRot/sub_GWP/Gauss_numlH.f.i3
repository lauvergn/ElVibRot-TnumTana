...
!...... variables Q    
       DO i=1,ND
         Y(i)=Qsym0(mole%liste_QactTOQsym(i))
...
!....COMPUTATION OF  deltaQ
     ...
     *
     *                         para_Tnum,mole)
c       CALL calc_d0d1d2g_G_vep(Qsym0,
c    *
c    *                          d0g,d1g,d2g,ecri_g,
c    *                          d0GG,d1GG,d2GG,ecri_GG,
c    *                          vep,
c    *
c    *                          para_Tnum,mole)
...
!CALCUL DE LA PHASE
       VTEMP=MATMUL(G,YP)
       E=0.5d0*DOT_PRODUCT(YP,VTEMP)-ENE0
...
!! Argument terme diagonal (uniquement) du nbre complex Z
       ...
       TRACE=0.d0
       do i=1,nd
         TRACE=TRACE+log(abs(Z(i,i)))+eye*arg(i)
       enddo
