...
SUBROUTINE init_EVR()
   USE mod_EVR
      IMPLICIT NONE
      logical  :: analysis_only,Popenmp,Popenmpi
...
SUBROUTINE get_TnumRefGeom_Q0(Q0,nb_Q0,Qdyn0)
  ...
  IF (Qdyn0) THEN
    Q0(:) = mole%tab_Qtransfo(mole%nb_Qtransfo)%ActiveTransfo%Qdyn0(:)
  ELSE ! Qact order
    Q0(:) = mole%tab_Qtransfo(mole%nb_Qtransfo)%ActiveTransfo%Qact0(:)
  END IF
...
SUBROUTINE Modify_TnumRefGeom_Q0(Q0,nb_Q0,Qdyn0)
  ...
  IF (Qdyn0) THEN
    mole%tab_Qtransfo(mole%nb_Qtransfo)%ActiveTransfo%Qdyn0(:) =  Q0(:)
    CALL Qdyn_TO_Qact_FROM_ActiveTransfo(Q0,Q1,mole%ActiveTransfo)
    mole%tab_Qtransfo(mole%nb_Qtransfo)%ActiveTransfo%Qact0(:) =  Q1(:)
  ELSE
    mole%tab_Qtransfo(mole%nb_Qtransfo)%ActiveTransfo%Qact0(:) =  Q0(:)
    CALL Qact_TO_Qdyn_FROM_ActiveTransfo(Q0,Q1,mole%ActiveTransfo)
    mole%tab_Qtransfo(mole%nb_Qtransfo)%ActiveTransfo%Qdyn0(:) =  Q1(:)
  END IF
...
SUBROUTINE get_nb_nq(nb,nq)
  ...
  nb = get_nb_FROM_basis(para_AllBasis%BasisnD)
...
!=====================================================================
      ...
      IF (associated(para_AllOp%tab_Op)) THEN
        DO i=1,size(para_AllOp%tab_Op)
          CALL dealloc_para_Op(para_AllOp%tab_Op(i),keep_init=.TRUE.)
        END DO
      END IF
