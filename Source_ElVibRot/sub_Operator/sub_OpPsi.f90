...
!-----------------------------------------------------------
      ...
      USE mod_system
      USE mod_Coord_KEO,       ONLY : get_Qact0, get_d0GG
  ...
  ELSE
     ...
     DO iblock=1,nb_block
        ...
        IF (para_Op%direct_KEO) THEN
          ...
          !$OMP  do
          DO iq=iq1,iq2
            CALL get_Qact0(Qact,para_Op%mole%ActiveTransfo) ! rigid, flexible coordinates
            CALL Rec_Qact(Qact(1:para_Op%mole%nb_act1),para_Op%para_AllBasis%BasisnD,iq,para_Op%mole)
