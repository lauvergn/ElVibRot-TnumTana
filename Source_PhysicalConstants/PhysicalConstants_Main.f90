...
!! <a href="http://physics.nist.gov/cuu/Constants/archive2006.html">NIST</a>
...
!!     mass = get_mass_Tnum(const_phys%mendeleev,Z=6,A=12)
 ...
 21   format (a,f18.6)
      ...
      write(out_unitp,*) 'mass 100. in au',mass
      CALL flush_perso(out_unitp)
      ...
      write(out_unitp,*) 'mass of D ("1_2") in au',mass
      CALL flush_perso(out_unitp)
      ...
      DO
        ...
        IF (err_read /= 0) EXIT
        write(out_unitp,*) 'mass_name: ',trim(adjustl(mass_name))
        mass = get_mass_Tnum(const_phys%mendeleev,name=mass_name,err_mass=err_mass)
        write(out_unitp,*) 'Tnum mass: ',mass
        write(out_unitp,*) ' -------------------------------------- '
        CALL flush_perso(out_unitp)
