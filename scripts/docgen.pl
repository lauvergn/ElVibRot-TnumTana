...
sub is_private{
    my $name = shift;
    if ($privacy_default eq 'private \i'){
...
#    print STDERR $line, "\n";
    ...
    if ($in_external_interface) {
        if ($line =~ m'^\s* end \s+ interface \s*$'xi){
            ...
            $external_interfaces .= "\n";
            if ( ($line =~ m'^\s* subroutine \s+ (\w+) \s*\( (.*) \)\s*$'xi) or
            ($line =~ m'^\s* .* \s* subroutine \s+ (\w+) \s*\( (.*) \)\s*$'xi)  or
            ($line =~ m'^\s* .* \s* function \s+ (\w+) \s*\( (.*) \)\s*$'xi)  or
            ($line =~ m'^\s* .* \s* function \s+ (\w+) \s*\( (.*) \) \s+ result \s+ (\w+) \s*\( (.*) \) \s*$'xi) ) {
    ...
    } elsif ($line =~ m'^\s*(public|private)\s*$'i){
        if ($in_data_env){
    ...
    # Evaluate assignment interface blocks
    } elsif ($line =~ m'^\s* interface \s+ assignment \s+ \( \s* = \s* \) \s* $'xi){
    ...
    } elsif ($line =~ m'^\s* interface \s+ operator \s+ \( \s* [-+*/]  \s* \) \s* $'xi){
        print ("  (assignment interface block)\n") if ($debug);
        $interface_name = "operator";
        if ($in_data_env){
            $in_generic_interface = 1;
            if ($description ne ""){
                $interfaces{$interface_name}{'description'} = $description;
    ...
    } elsif ($line =~ m'^\s* end \s+ interface'xi){
        ...
        $line = <INFILE>;
    } elsif ($line =~ m'^\s* module \s+ procedure \s+ (.*)$'xi){
    ...
    # This handles such lines as
    #   subroutine init_filter_fermi (f, t, lambda, w_0)
    # or
    ...
    ($line =~ m'^\s* (subroutine \s* \w+) \s*\( (.*) \)\s*$'xi) or
    ($line =~ m'^\s* (.*\s* subroutine \s* \w+) \s*\( (.*) \)\s*$'xi) or
    ($line =~ m'^ \s* ( .* \s* function \s* \w+ ) \s* \( (.*?) \) \s+ result \s* \( (\w+) \) \s* $'xi) or
    ($line =~ m'^\s* (.*\s* function \s+ \w+) \s*\( (.*) \)\s*$'xi)) {
        print ("  (routine start)\n") if ($debug);
        $long_routine_name = $1;
        my $routine_params = $2;
        $external_interfaces = "";
        if ($long_routine_name =~ m'^\s* (.*) (function \s+ (\w+))'xi){
    ...
    # The line provides intent, shape, and type information
    } elsif ($line =~ m'^\s* (.*) \s* :: \s* (.*)$'xi){
        print ("  (param definition)\n") if ($debug);
        my $param_lhs = $1;
        my $param_rhs = $2;
        my $param_name = '';
        my $param_shape = '';
        if ($param_lhs =~ m'^(public|private)'i){
            $line = <INFILE>;
            print ("  (skipped)\n") if ($debug);
            next;
        }
        foreach my $param_name (split(/\s*,\s\s*/, $param_rhs)){
            ...
            if ($param_type =~ m'intent\(\s*(in|out|inout)\s*\)'i){
    ...
    # routine documentation
    } elsif ($line =~ m'^\s*end\s+(subroutine|function)'i){
        print ("  (end routine)\n") if ($debug);
        ...
        if ($routine_name =~ m'\s*(function|subroutine)\s*'i){
...
if (keys(%routines) > 0){
    ...
    foreach my $routine_name (keys(%routines)){
        ...
        if ($routines{$routine_name}{'long_name'} =~ /subroutine/i){
            $global_index->{$routine_name} = "subroutine;$outfile#$routine_name";
        } elsif ($routines{$routine_name}{'long_name'} =~ /function/i){
