...
!-----------------------------------------------------------
      ...
      IF (Rkind /= real64 .AND. (type_diag_loc == 3 .OR. type_diag_loc == 4)) type_diag_loc = 1
...
SUBROUTINE tred2_nr(a,d,e,n)
    USE mod_system
    IMPLICIT NONE

    integer,                          INTENT(IN)    :: n
    REAL(kind=Rkind), DIMENSION(n,n), INTENT(INOUT) :: a
    REAL(kind=Rkind), DIMENSION(n),   INTENT(OUT)   :: d,e

    INTEGER :: i,j,l
    REAL(kind=Rkind) :: f,g,h,hh,scale
    REAL(kind=Rkind), DIMENSION(n) :: gg
    real(kind=Rkind) :: outerprod ! function
    LOGICAL :: yesvec
    ...
    END SUBROUTINE tred2_nr

    SUBROUTINE tqli_nr(d,e,z,n)
    USE mod_system
    IMPLICIT NONE

    integer,                                    INTENT(IN)    :: n
    REAL(kind=Rkind), DIMENSION(n),             INTENT(INOUT) :: d,e
    REAL(kind=Rkind), DIMENSION(n,n),           INTENT(INOUT) :: z

    INTEGER :: i,iter,l,m,ndum
    REAL(kind=Rkind) :: b,c,dd,f,g,p,r,s
    REAL(kind=Rkind), DIMENSION(n) :: ff
    real(kind=Rkind) :: pythag ! function
    ...
    END SUBROUTINE tqli_nr

FUNCTION pythag(a,b)
USE mod_system
    IMPLICIT NONE

    REAL(kind=Rkind) :: a,b,pythag
      ! Computes (a**2 + b**2)**1/2 without destructive underflow or overflow.
    REAL(kind=Rkind) :: absa,absb
...
END
FUNCTION outerprod(a,b,n)
  USE mod_system
  integer,                        INTENT(IN) :: n
  REAL(kind=Rkind), DIMENSION(n), INTENT(IN) :: a,b
  REAL(kind=Rkind), DIMENSION(n,n)           :: outerprod
